...
function insertPhotoToAlbum($pdo, $album_id, $photo_id) {
    $sql = "INSERT INTO album_photos (album_id, photo_id) VALUES (?, ?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$album_id, $photo_id]);
...
function editAlbum($pdo, $albumName, $albumId) {
    $sql = "UPDATE albums SET album_name = ? WHERE album_id = ?";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute([$albumName, $albumId]);
}

function getAlbumById($pdo, $albumId) {
...
function getAllAlbumsByUser($pdo, $username) { 
    $sql = "SELECT * FROM albums WHERE username = ?"; 
    $stmt = $pdo->prepare($sql); 
    $stmt->execute([$username]); 
    return $stmt->fetchAll(PDO::FETCH_ASSOC); }

    function getPhotosByAlbum($pdo, $albumId) { 
    ...
    function deleteAlbum($pdo, $albumId, $username) {
        $deleteAlbumPhotosSql = "DELETE FROM album_photos WHERE album_id = ?";
        $stmt = $pdo->prepare($deleteAlbumPhotosSql);
        $stmt->execute([$albumId]);
    
        $deleteAlbumSql = "DELETE FROM albums WHERE album_id = ? AND username = ?";
        $stmt = $pdo->prepare($deleteAlbumSql);
        
        return $stmt->execute([$albumId, $username]);
    }
